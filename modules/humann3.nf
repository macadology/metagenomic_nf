...
process HUMANN3 {
    ...
    output:
    publishDir "$params.outputdir/${prefix}/humann3", mode: 'copy'
    ...
    humann --input ${prefix}.fq.gz --output . --threads ${task.cpus} --protein-database $humannDB_Uniref --nucleotide-database $humannDB_Chocophlan --metaphlan-options '--bowtie2db $humannDB_bt2Chocophlan --index $params.humannDB_index --nproc $params.humannThreads' --bowtie-options '--threads ${task.cpus}' --diamond-options '--threads ${task.cpus}'
    rm ${prefix}.fq.gz
